    ...
    node {
      ...
      withEnv(env) {
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /documentchain-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "krekeler-documentchain/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from krekeler-documentchain/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'krekeler-documentchain/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from krekeler-documentchain/develop"
            }
          }

          if (hasCache) {
            sh "cd /documentchain-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /documentchain-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /documentchain-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /documentchain-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /documentchain-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /documentchain-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /documentchain-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
